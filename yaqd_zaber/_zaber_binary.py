...
class ZaberBinary(ContinuousHardware):
    ...
    def __init__(self, name, config, config_filepath):
        ...
        if config["serial_port"] in ZaberBinary.serial_dispatchers:
            ...
            self._serial = SerialDispatcher(BinarySerial(config["serial_port"], config["baud_rate"]))
    ...
    def _set_position(self, position):
        self.serial.write(BinaryCommand(self._axis, 20, round(position)))
    ...
    def direct_serial_write(self, command):
        ...
        if len(command) <= BinarySerial.MESSAGE_LENGTH - 1:
            command = bytes([self._axis]) + command
        command = command.ljust(BinarySerial.MESSAGE_LENGTH, '\0')
